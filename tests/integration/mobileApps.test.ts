...
describe('mobile apps integration', () => {
  beforeEach(() => {
    (db.query as jest.Mock).mockReset();
    (getPlanEntitlements as jest.Mock).mockReset();
  });

  it('blocks mobile apps when entitlements are disabled', async () => {
    (getPlanEntitlements as jest.Mock).mockResolvedValue({ features: { mobile_apps: false } });
  ...
  it('creates mobile apps when entitlements are enabled', async () => {
    (getPlanEntitlements as jest.Mock).mockResolvedValue({ features: { mobile_apps: true } });
    (db.query as jest.Mock).mockImplementation((sql: string) => {
      ...
      if (sql.includes('INSERT INTO mobile_apps')) {
        return Promise.resolve({ rows: [{ id: 'app-1', name: 'OkLeaf iOS', platform: 'ios' }] });
      }
    ...
    const res = await request(app)
      ...
      .set('Authorization', `Bearer ${token}`)
      .send({ name: 'OkLeaf iOS', platform: 'ios', scheme: 'okleaf://' });
